      ...
      <view class="calendar_meeting">
        ...
        >
          <view class="create_content_box">
            ...
            <view class="meeting_content_name">{{ item.projectName }}</view>
          </view>
        </view>

        <view v-if="meetingDetail" class="meeting_detail" :class="meetingDetail.class" :style="meetingDetail.style">
          <view>{{ meetingDetail.userName }}</view>
          <view>{{ meetingDetail.time }}</view>
          <view>{{ meetingDetail.projectName }}</view>
          <view>医生： {{ meetingDetail.doctorName }}</view>
        </view>
        ...
        >
          ...
          <view class="create_content_box">
            ...
            <text class="meeting_content">{{ createMeet.projectName }}</text>
          </view>
        </view>
      </view>
      <!-- 时间线刻度 -->
...
export default {
  ...
  // },
  watch: {
    chooseDateProp: function() {
      let nowDate = this.$timeFormat(new Date(), 'yyyy/MM/dd');
      if (this.$props.chooseDateProp !== nowDate) {
        this.isToday = -2;
      } else {
        this.isToday = 0;
      }
  ...
  methods: {
    ...
    async getScheduleList() {
      let scheduleList = this.$props.scheduleListProp;
      // let scheduleList = this.scheduleListState;
      let list = [];
      for (let i = 0; i < scheduleList.length; i++) {
        let start = scheduleList[i].startTime.substring(11, 16).split(':');
        let end = scheduleList[i].endTime.substring(11, 16).split(':');
        ...
        list.push({
          ...
          style: height + top,
          projectName: scheduleList[i].projectName,
          time: scheduleList[i].startTime.substring(5, 16) + '-' + scheduleList[i].endTime.substring(11, 16),
          isFlex: isFlex,
          startId: st,
          endId: ed,
          bgClass: bgClass
    ...
    showDetail(index, e) {
      ...
      let newScheduleList = this.$props.scheduleListProp[index]
      let newShow = this.scheduleList[index],
        detailClass = '',
        style = '',
        doctorName = '',
        userName = '';
      ...
      this.meetingDetail = {
        ...
        class: detailClass,
        style: style,
        projectName: newShow.projectName,
        time: newShow.time,
        doctorName: newScheduleList.doctorName,
        userName: newScheduleList.userName
      };
    },
    ...
    hasMeeting(idSt, idEnd) {
      let hasMeeting = this.scheduleList.some(function(item) {
        return (
    ...
    touchMeetingStart(e) {
      ...
      self.meetingTouchStartY = e.changedTouches[0].clientY;
    ...
    touchMeetingEnd(e) {
      ...
      if (absY == 0) {
        ...
        let params = {
          ...
          date: this.$props.chooseDateProp
        };
