...
export default {
  ...
  async onLoad(options) {
    ...
    let index = 0;
    const rawList = [];
    this.allMessage = messagelist.list;
    messagelist.list.reverse().forEach(item => {
      const message = {
        ...
        url: item.url,
        nickName:
          uni.getStorageSync('userInfo').id == item.userId ? uni.getStorageSync('userInfo').nickname : receiverInfo.nickname,
        avatar:
  ...
  methods: {
    ...
    async lookMoreMessage() {
      ...
      this.hasMore = moreMessagelist.list.length > 0 ? true : false;
      const rawList = [];
      moreMessagelist.list.reverse().forEach(item => {
        const message = {
          ...
          url: item.url,
          nickName:
            uni.getStorageSync('userInfo').id == item.userId ? uni.getStorageSync('userInfo').nickname : _this.receiverName,
