	...
	<view class="my">
		...
		<view class="top-background  d-flex ">
			<view class="user_img position-absolute ">
				<u--image v-if="patientDetail.gender === '男'"
					src="https://oss-augustrush.oss-cn-shenzhen.aliyuncs.com/man.png" width="70" height="70"
					shape="circle"></u--image>
				<u--image v-if="patientDetail.gender === '女'"
					src="https://oss-augustrush.oss-cn-shenzhen.aliyuncs.com/woman.png" width="70" height="70"
					shape="circle"></u--image>
			</view>
			<view class="mt-4 ml-3">
				<view class="userInfo" style="width: 500rpx;">
					...
					<view class="d-flex userInfo-row2 j-sb mt-2">
						...
						<view class="a-center pr-5" style="border-right: 2px solid #2586fe;">
							<view class="label mr-2 font-color font-weight font-md"
				...
				<view class="consumption">
					<view class="d-flex">
						<view class="d-flex j-start mr-5 line-h">
							...
							<view class="font-md font-weight" style="color: #00cc9a;">{{ getPrice+3000}}.00 元</view>
						</view>
						<view class="d-flex j-end line-h">
							<text class="mr-1">欠费:</text>
							<view class="font-md font-weight" style="color: #c54748;">{{ getPrice}}.00 元</view>
						</view>
					</view>
				</view>
			</view>
		...
		<uni-card>
			<view class="title d-flex flex-row j-sb  a-center border-bottom pb-1">
				<view>阶段:</view>
				<u-tag :text="patientDetail.stage + '阶段'" plain v-if="patientDetail.stage == '第一阶段'" shape="circle">
				</u-tag>
				<u-tag :text="patientDetail.stage + '阶段'" plain v-if="patientDetail.stage == '第二阶段'" shape="circle"
					type="warning"></u-tag>
				<u-tag :text="patientDetail.stage + '阶段'" plain v-if="patientDetail.stage == '第三阶段'" shape="circle"
					type="success"></u-tag>
			</view>
		...
		<view class="mt-3 mx-3">
			<u-button type="primary" text="确认入诊" @click="handleConfirm(patientDetail.id)" v-if="isShowBtn">
			</u-button>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			async getPatientDetail(id, orderNumber) {
				...
				const {
					data: res
				} = await this.$http.post('/doctor/getRecordDet', {
					id: id,
